...
import { COOKIE_KEYS, CUSTOM_STORE_KEYS } from "./enums/keys";
import { getCookie, isRunningOnClient, setCookie, isValidObject, parseCookieValue } from "../helpers/utils";
import { isDeeplyDifferent, validateSliceUpdate } from "./utils/util";
...
class ApplicationStore {
     ...
     */
    get orderingSource() {
        return (this.constructorOrderingSource ||
            this.skyfireOrderingSource ||
            'storefront');
    }
    /** orderingSourceType — 'storefront' when an ordering source is present. */
    get orderingSourceType() {
        return this.orderingSource ? 'storefront' : '';
    }
    constructor(initialData, authorizationHeader, domain, orderingSource) {
        this.PaymentGateways = PaymentGatway;
        /** orderingSource provided via the constructor (initialised by react-starter). */
        this.constructorOrderingSource = '';
        ...
        this.setSliceData = (sliceName, data) => {
            const state = this.store.getState();
            if (!validateSliceUpdate(state, sliceName)) {
                return;
            }
            this.dispatch({
                type: `${sliceName}/setData`,
                payload: data,
            });
        };
        /**
         ...
         */
        this.handleGQLResponse = (responseData, params = null, operationType, options) => {
        ...
        this.constructorOrderingSource = orderingSource || '';
        this.getters = {
            ...
            // SELLER DETAILS
            SELLER_DETAILS: (store) => store.custom[CUSTOM_STORE_KEYS.SELLER_DETAILS],
            i18N_DETAILS: (store) => store.custom[CUSTOM_STORE_KEYS.I18N_DETAILS],
            LOCATION_DETAILS: (store) => store.custom[CUSTOM_STORE_KEYS.LOCATION_DETAILS],
            // Custom Getters
        ...
        // Initialize i18n and location details from cookies
        let i18nDetails = {};
        let locationDetails = {};
        if (isRunningOnClient()) {
            i18nDetails = parseCookieValue(getCookie(COOKIE_KEYS.I18N_DETAILS));
            locationDetails = parseCookieValue(getCookie(COOKIE_KEYS.LOCATION_DATA));
        }
        else {
            i18nDetails = parseCookieValue(this.serverCookies[COOKIE_KEYS.I18N_DETAILS]);
            locationDetails = parseCookieValue(this.serverCookies[COOKIE_KEYS.LOCATION_DATA]);
        }
        this.custom.setValue(CUSTOM_STORE_KEYS.I18N_DETAILS, i18nDetails);
        this.custom.setValue(CUSTOM_STORE_KEYS.LOCATION_DETAILS, locationDetails);
    }
    executeFetch(query, variables) {
        return __awaiter(this, void 0, void 0, function* () {
            // Ensure app_i18n_details and location_details cookies are present, sync from state if missing
            const state = this.store.getState();
            const i18nDetailsFromState = state.custom[CUSTOM_STORE_KEYS.I18N_DETAILS];
            const locationDetailsFromState = state.custom[CUSTOM_STORE_KEYS.LOCATION_DETAILS];
            if (isRunningOnClient()) {
                // Client-side: Sync cookies from Redux state if missing
                // Sync i18n details
                const parsedI18nCookie = parseCookieValue(getCookie(COOKIE_KEYS.I18N_DETAILS));
                if (!isValidObject(parsedI18nCookie) && isValidObject(i18nDetailsFromState)) {
                    setCookie(COOKIE_KEYS.I18N_DETAILS, i18nDetailsFromState, 1);
                }
                // Sync location details
                const parsedLocationCookie = parseCookieValue(getCookie(COOKIE_KEYS.LOCATION_DATA));
                if (!isValidObject(parsedLocationCookie) && isValidObject(locationDetailsFromState)) {
                    setCookie(COOKIE_KEYS.LOCATION_DATA, locationDetailsFromState, 1);
                }
            }
            else {
                // Server-side: Sync this.serverCookies from Redux state if missing or invalid
                // Sync i18n details
                const parsedI18nCookie = parseCookieValue(this.serverCookies[COOKIE_KEYS.I18N_DETAILS]);
                if (!isValidObject(parsedI18nCookie) && isValidObject(i18nDetailsFromState)) {
                    // Store as string (serverCookies expects string values)
                    this.serverCookies[COOKIE_KEYS.I18N_DETAILS] = JSON.stringify(i18nDetailsFromState);
                }
                // Sync location details
                const parsedLocationCookie = parseCookieValue(this.serverCookies[COOKIE_KEYS.LOCATION_DATA]);
                if (!isValidObject(parsedLocationCookie) && isValidObject(locationDetailsFromState)) {
                    // Store as string (serverCookies expects string values)
                    this.serverCookies[COOKIE_KEYS.LOCATION_DATA] = JSON.stringify(locationDetailsFromState);
                }
            }
            const requestHeaders = Object.assign(Object.assign({ "Content-Type": "application/json", Authorization: this.authorizationHeader }, (this.orderingSource
                ? {
                    'x-ordering-source': this.orderingSource,
                    'x-ordering-source-type': 'storefront',
                }
                : {})), (isClient
                ? {
                    credentials: "include",
    ...
    setServerCookies(cookies = {}) {
        ...
        // Parse and update store with server cookies
        const i18nDetails = parseCookieValue(cookies[COOKIE_KEYS.I18N_DETAILS]);
        const locationDetails = parseCookieValue(cookies[COOKIE_KEYS.LOCATION_DATA]);
        // Update Redux state with parsed cookie values (only if valid objects)
        if (isValidObject(i18nDetails)) {
            this.custom.setValue(CUSTOM_STORE_KEYS.I18N_DETAILS, i18nDetails);
        }
        if (isValidObject(locationDetails)) {
            this.custom.setValue(CUSTOM_STORE_KEYS.LOCATION_DETAILS, locationDetails);
        }
    }
    setI18nDetails(cookieValue) {
        try {
            // Parse cookieValue using helper function for consistency
            const parsedCookieValue = parseCookieValue(cookieValue);
            // Parse current location data using helper function
            let currentLocationData = parseCookieValue(getCookie(COOKIE_KEYS.LOCATION_DATA));
            if (parsedCookieValue.countryCode == currentLocationData.country_iso_code) {
                currentLocationData.country_iso_code =
                    parsedCookieValue.countryCode || currentLocationData.country_iso_code;
                currentLocationData.country =
                    parsedCookieValue.countryName || currentLocationData.country;
            }
            else {
                currentLocationData = {
                    country_iso_code: parsedCookieValue.countryCode || currentLocationData.country_iso_code,
                    country: parsedCookieValue.countryName || currentLocationData.country,
                };
            }
            setCookie(COOKIE_KEYS.LOCATION_DATA, currentLocationData, 1);
            setCookie(COOKIE_KEYS.I18N_DETAILS, parsedCookieValue, 1);
            // Only update Redux state if parsed values are valid objects
            if (isValidObject(parsedCookieValue)) {
                this.custom.setValue(CUSTOM_STORE_KEYS.I18N_DETAILS, parsedCookieValue);
            }
            if (isValidObject(currentLocationData)) {
                this.custom.setValue(CUSTOM_STORE_KEYS.LOCATION_DETAILS, currentLocationData);
            }
        }
        catch (error) {
    ...
    // temp revert till skyfire is either fully reverted or implemented
    setOrderingSource(orderingSource) {
        // Store the skyfire-provided value. A falsy/non-string value means the
        // effective orderingSource getter falls back to the constructor value.
        this.skyfireOrderingSource =
            typeof orderingSource === 'string' ? orderingSource : '';
        // The theme (constructor) value takes precedence. If skyfire also supplies
        // one while the theme already has a value, warn that the platform-set
        // ordering source will be ignored.
        if (this.skyfireOrderingSource && this.constructorOrderingSource) {
            console.warn('Ordering source set from platform is not applicable as the theme ordering source is taking precedence.');
        }
    }
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                for (const name of operationNames) {
                    if (name === "Theme") {
                        mergedAst = this.gqlModifier.modifyThemeQuery(mergedAst);
                    }
                ...
                this.custom.setValue(CUSTOM_STORE_KEYS.SELLER_DETAILS, sellerDetails);
