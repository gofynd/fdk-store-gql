...
export default class PotleePayment {
    constructor(gqClient, selectedPayment) {
    ...
    verifyUser(params = {}) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            if (self.paymentFlow.data.gateway.is_customer_validation_required) {
                window.potlee.generateFingerprint({
                    mobile: self.paymentFlow.data.gateway.sdk.data.user_phone,
                    countryCode: "91",
                }, function (fingerpoint) {
                    return resolve(fingerpoint);
                });
            }
        })
            .then(function (res) {
            params.fingerprint = res;
    ...
    fetchAddressForApproval(params) {
        ...
        return new Promise((resolve, reject) => {
            let { address_id, billing_address_id, bag_data, fingerprint } = params;
            ...
            const pArr = address_id === billing_address_id
                ? [
                    this.gqClient.query({
                        query: gql `
                          query Address{
                            getAddressById:address(id: "${address_id}") {
                              custom_json
                              address
                              address_type
                              area
          ...
          `,
            ...
            Promise.all(pArr)
                .then((dat) => {
                const data = dat[0].data;
                const delivery_address = {
                    line1: data[0].data.address,
                    line2: data[0].area,
                    city: data[0].city,
                    state: data[0].state,
    ...
    checkPotleeApproval(data) {
        ...
        options.phone_number = this.paymentFlow.data.gateway.sdk.data.user_phone;
        return new Promise((resolve, reject) => {
            this.gqClient
                .query({
                query: gql `
