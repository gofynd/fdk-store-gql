...
import { STORE_KEYS, COOKIE_KEYS, CUSTOM_STORE_KEYS } from "../enums/keys";
...
const logisticSlice = createSlice({
    ...
    reducers: {
        ...
        setlocality(state, action) {
            var _a;
            const { countryDetails, localities, locality, localityValue, meta } = action.payload;
            ...
            const locationData = Object.assign(Object.assign(Object.assign(Object.assign({}, (locationDataCookie || {})), (locality && localityValue ? { [locality]: localityValue } : {})), locationObj), meta);
            setCookie(COOKIE_KEYS.LOCATION_DATA, locationData, 1);
            setTimeout(() => {
                emitFPIEvent(FPI_EVENTS.LOCATION_DETAILS, action.payload, locationData, {});
                window.fpi.custom.setValue(CUSTOM_STORE_KEYS.LOCATION_DETAILS, locationData);
            }, 0);
            state.pincode_details = action.payload;
        }
    },
    extraReducers: (builder) => {
        builder.addCase(`${STORE_KEYS.LOGISTIC}/setData`, (state, action) => {
